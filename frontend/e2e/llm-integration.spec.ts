...
const MOCK_SCENE_RESPONSE = {
  ...
  scene: {
    sceneIndex: 1,
    themeId: 'focus',
    narrative: '「道徳」をテーマに、重要な決断を迫られました。あなたはどのようにアプローチしますか？',
    choices: [
...
async function setupLLMApiMocks(page: Page) {
  ...
  await page.route('**/api/sessions/*/scenes/*/choice', async (route) => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        sessionId: MOCK_SESSION_ID,
        nextScene: null,  // 4シーン目完了を想定
        sceneCompleted: true
      }),
    });
  });

  // Results API - 正しいエンドポイント
...
test.describe('LLM Integration E2E Tests', () => {
  ...
  test('should complete full user journey from keyword to result', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="scene-narrative"]')).toContainText('道徳');
  ...
  test('should handle LLM keyword generation successfully', async ({ page }) => {
    ...
    // 各キーワードが「ど」から始まることを確認（実際のレスポンスに合わせて調整）
    for (let i = 0; i < 4; i++) {
      const keyword = await keywordButtons.nth(i).textContent();
      // キーワードボタンにはラベルテキストが含まれるため、単純な文字チェックではなく存在確認に変更
  ...
  test('should display scene content generated by LLM', async ({ page }) => {
    ...
    await expect(narrative).toContainText('道徳');
    await expect(narrative).toContainText('重要な決断');
  ...
  test('should handle fallback when LLM is unavailable', async ({ page }) => {
    ...
    // フォールバックキーワードの確認
    const keywordButtons = page.locator('[data-testid="keyword-candidates"] button');
    await expect(keywordButtons.first()).toContainText('道徳');
  ...
  test('should maintain session state across LLM interactions', async ({ page }) => {
    ...
    // セッション情報があれば確認（実際のセッションIDは動的なのでパターンチェック）
