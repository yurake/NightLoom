...
## 6. ユースケース概要
### UC-01: 診断プレイ
1. ユーザーがアプリにアクセスすると、バックエンドが50音から1文字をランダム抽出し、評価軸生成ロジック (§10) を即時実行する。
2. 抽出した文字と評価軸メタを用いて、同じ文字で始まる単語候補 4 件を提示し、ユーザーに候補選択または任意入力を促す。
3. 選ばれた単語を初期評価情報として記録し、シーン1のシナリオをその単語に基づいて生成する。
4. 以降は 4 シーンを順番に表示し、各シーンでユーザーが 1 選択肢を選ぶ。
5. 内部スコア集計・最終計算を実施する。
6. タイプ分類・スコア結果を表示する。
7. セッション終了（データ破棄）。

### 主要ユーザフロー (単一路線)
access → initialPrompt → scene1 → scene2 → scene3 → scene4 → result
...
| FR-026 | シーン連動UIテーマ | シナリオ内容に合わせ結果画面デザインを切替 | Medium (次回開発) |
...
- 初期アクセス時に提示された単語を導入語として採用し、シーン1の状況描写と選択肢に反映する。
- セッション開始時に統一テーマを決定し、各シーンで一貫したコンテキストを保つ。
- LLM 応答が失敗した場合は固定シナリオへ切り替える。
- 詳細設計: docs/design/overview.md §3.1, §3.5, §6.2

## 10. 評価軸生成要件
- 軸数は 2〜6 の範囲とし、名称・説明・方向性をセットで提供する。
- 軸の重みは選択肢ごとに配列で返却し、後段で正規化する。
- 初期単語が各評価軸に与える影響度を算定し、初期スコアまたは補正係数として蓄積する。
- 範囲外やエラー時は既定 2 軸へフォールバックする。
- 詳細設計: docs/design/overview.md §3.2, §3.4
...
- 初期アクセス時に選ばれた単語のメタ情報を評価軸スコア計算に組み込み、重み付けまたはバイアスとして反映する。
...
| FR-026 | シーン連動UIテーマ(§9), docs/design/overview.md §3.5 | 次回開発対象 
